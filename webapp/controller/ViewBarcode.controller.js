        ...
        */
        ...
        return Controller.extend("zbarcodescan.controller.ViewBarcode", {
            ...
            onScanSuccess: function (oEvent) {
                ...
                const scannedText = oEvent.getParameter("text");

                var oBundle = this.getOwnerComponent().getModel("i18n").getResourceBundle();

                if (!scannedText) {
                    oScanResultText.setValue('');
                    MessageToast.show("Scan returned empty");
                    return;
                }

                const cleanText = scannedText.replace(/[\r\n\t]/g, "");
                this.oScanResultText.setValue(cleanText);

                if (cleanText.length >= 18 && cleanText.length <= 25) {
